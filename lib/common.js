...
function getIds (joinedIds, idField, country, lang, requestOptions, limit) {
  idField = idField || 'id';
  country = country || 'us';
...
function lookup (ids, idField, country, lang, requestOptions, limit) {
  ...
  const idsLength = ids.length / LIMIT_LENGTH;
  const joinedIdsTasks = [];

  for(let y = 0; y < idsLength; y++) {
    const strIds = ids.splice(0, LIMIT_LENGTH).join(',');
    joinedIdsTasks.push(
        getIds(strIds, idField, country, lang, requestOptions, limit),
    );
  }

  return Promise.all(joinedIdsTasks)
      .then((res) => res.flat());
}
...
function getXAppleStoreFront(country) {
  return `${c.markets[country] || 143441}-${c.languages[country] || 2},29`;
}

module.exports = { cleanApp, lookup, request: doRequest, storeId, getXAppleStoreFront };
