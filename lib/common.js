...
function getIds (joinedIds, idField, country, lang, requestOptions) {
  idField = idField || 'id';
  country = country || 'us';
...
function lookup (ids, idField, country, lang, requestOptions) {
  ...
  for(let y = 0; y < idsLength; y++) {
    ...
    joinedIdsTasks.push(
        getIds(strIds, idField, country, lang, requestOptions),
    );
  }
...
function getXAppleStoreFront(country) {
  return `${c.markets[country] || 143441}-${c.languages[country] || 1},29`;
}
